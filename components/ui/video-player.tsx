        ...
        <AnimatePresence>
          {isMuted && (
            <motion.button
              ...
              className="absolute top-4 right-4 z-20 flex items-center gap-2 px-4 py-2 rounded-full bg-[#111111d1] text-white backdrop-blur-md border border-white/10 hover:bg-[#111111f0] transition-colors shadow-[0_8px_32px_rgba(0,0,0,0.35)]"
              initial={{ opacity: 0, y: 10, filter: "blur(6px)" }}
              animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}
              exit={{ opacity: 0, y: 10, filter: "blur(6px)" }}
            >
              <VolumeX className="h-5 w-5" />
              <span className="text-sm font-medium">Unmute for sound</span>
            </motion.button>
          )}
        </AnimatePresence>
      )}
